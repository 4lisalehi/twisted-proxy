...
$(obj).target/deps/cares/cares.so: GYP_LDFLAGS := $(LDFLAGS_$(BUILDTYPE))
$(obj).target/deps/cares/cares.so: LIBS := $(LIBS)
$(obj).target/deps/cares/cares.so: LD_INPUTS := $(OBJS)
$(obj).target/deps/cares/cares.so: TOOLSET := $(TOOLSET)
$(obj).target/deps/cares/cares.so: $(OBJS) FORCE_DO_CMD
	$(call do_cmd,solink)

all_deps += $(obj).target/deps/cares/cares.so
# Add target alias
.PHONY: cares
cares: $(builddir)/cares.so

# Copy this to the shared library output path.
$(builddir)/cares.so: TOOLSET := $(TOOLSET)
$(builddir)/cares.so: $(obj).target/deps/cares/cares.so FORCE_DO_CMD
	$(call do_cmd,copy)

all_deps += $(builddir)/cares.so
# Short alias for building this shared library.
.PHONY: cares.so
cares.so: $(obj).target/deps/cares/cares.so $(builddir)/cares.so

# Add shared library to "all" target.
.PHONY: all
all: $(builddir)/cares.so
